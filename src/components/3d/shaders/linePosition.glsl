...
vec3 linePosition(
	...
	sampler2D prevPositionTexture, 
	inout vec3 transformed,
	float uMoveToStart,
	sampler2D defaultPositionTexture
) {
	...
	vec4 defaultPosition = texture2D(defaultPositionTexture, positionUV);
    ...
    // vColor = vec4(positionUVS, 0.0, 0.0, 1.0);
